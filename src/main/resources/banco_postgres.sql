...
comment on table TABULACAO is 'TABELA PARA CONTROLAR STATUS DE LIAGACOES, DOOU = DOADOR QUIZ DOR E OPERADOR PRODUZIU RECIBOS PARA RECEBER, NAO DOOU = DOADOR NAO QUIZ DOAR, TELEFONE OCUPADO , CAIXA POSTAL';
comment on column TABULACAO.DESCRICAO is 'DOOU = DOADOR QUIZ DOR E OPERADOR PRODUZIU RECIBOS PARA RECEBER, NAO DOOU = DOADOR NAO QUIZ DOAR, TELEFONE OCUPADO , CAIXA POSTAL';
...
CREATE TABLE CONTRIBUINTE (
	...
	ID_TABULACAO INTEGER REFERENCES TABULACAO (ID) NOT NULL,
...
comment on table RECIBOS is 'TABELA ONDE ENCONTRA-SE TODOS RECIBOS PRODUZIDOS';
comment on column RECIBOS.VALOR is 'VALOR DOACAO';
comment on column RECIBOS.STATUS is 'B = BAIXADO, G = GERADO, R = REAGENDADO, C = CANCELADO';
comment on column RECIBOS.DATA_OPERACAO is 'DATA PRODUCAO RECIBO';
comment on column RECIBOS.DATA_VENCIMENTO is 'DATA VENCIMENTO RECIBO';
comment on column RECIBOS.DATA_BAIXA is 'QUANDO OUVE BAIXA, CANCELAMENTO OU REAGENDAMENTO';
comment on column RECIBOS.PARCELA is 'QUANTIDADE PARCELAS PRODUZAIDAS 1/N';
